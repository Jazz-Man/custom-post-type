...
class CustomPostType
{
    ...
    public function __construct($post_type_names, array $options = [])
    {
        ...
        $this->post_type = sanitize_title($this->post_type_name);
    ...
    public function register_post_type()
    {
        ...
        $defaults = [
          'labels' => $labels, 'public' => true, 'show_in_rest' => true,
        ];
    ...
    public function register_taxonomy($taxonomy_names, array $options = [])
    {
        ...
        $taxonomy_name = sanitize_title($taxonomy_names);
        ...
        $defaults = [
          'labels' => $labels, 'hierarchical' => true, 'show_in_rest' => true,
        ];
            ...
            if (isset($vars['post_type']) && $this->post_type === $vars['post_type']) {
                if (isset($vars['orderby']) && $meta_key === $vars['orderby']) {
                    $vars = array_merge($vars, [
                      'meta_key' => $meta_key, 'orderby' => $orderby,
                    ]);
                }
            }
        }
    ...
    public function updated_messages($messages)
    {
        ...
        $messages[$this->post_type_name] = [
          0 => '', 1 => sprintf(__('%s updated.', $this->textdomain), $this->singular), 2 => __('Custom field updated.',
            $this->textdomain), 3 => __('Custom field deleted.', $this->textdomain), 4 => sprintf(__('%s updated.',
            $this->textdomain), $this->singular),
    ...
    public function bulk_updated_messages($bulk_messages, $bulk_counts)
    {
        $bulk_messages[$this->post_type_name] = [
          'updated'                                                                                         => _n("%s {$this->singular} updated.",
            "%s {$this->plural} updated.", $bulk_counts['updated']),
          ...
          'deleted'                                                                                         => _n("%s {$this->singular} permanently deleted.",
            "%s {$this->plural} permanently deleted.", $bulk_counts['deleted']),
          'trashed'                                                                                         => _n("%s {$this->singular} moved to the Trash.",
            "%s {$this->plural} moved to the Trash.", $bulk_counts['trashed']),
